      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-3xl shadow-card p-8">
          ...
          <div className="relative w-64 h-64 mx-auto mb-8">
            <div className="absolute inset-0 rounded-full bg-gradient-to-br from-purple-100 to-pink-100"></div>
            {locationAnalytics.map((location, index) => {
              const startAngle = locationAnalytics.slice(0, index).reduce((sum, loc) => sum + (loc.percentage * 3.6), 0);
              const endAngle = startAngle + (location.percentage * 3.6);
              return (
                <div
                  key={location.city}
                  className="absolute inset-0 rounded-full"
                  style={{
                    background: `conic-gradient(from ${startAngle}deg, ${location.color} 0deg, ${location.color} ${location.percentage * 3.6}deg, transparent ${location.percentage * 3.6}deg)`,
